#include "AP_OpenMV_MAVliteMsgHandler.h"
...
void AP_OpenMV_MAVliteMsgHandler::handle_command_long(const AP_OpenMV_MAVlite_Message &rxmsg)
{
    ...
    uint8_t param_count = AP_OpenMV_MAVlite_Message::bit8_unpack(cmd_options, 3, 0);               // first 3 bits
...
MAV_RESULT AP_OpenMV_MAVliteMsgHandler::handle_command(const mavlink_command_long_t &mav_command_long)
{
...
MAV_RESULT AP_OpenMV_MAVliteMsgHandler::handle_command_do_set_mode(const mavlink_command_long_t &mav_command_long)
{
...
void AP_OpenMV_MAVliteMsgHandler::send_command_ack(const MAV_RESULT mav_result, const uint16_t cmdid)
{
    AP_OpenMV_MAVlite_Message txmsg;
...
MAV_RESULT AP_OpenMV_MAVliteMsgHandler::handle_command_preflight_calibration_baro(const mavlink_command_long_t &mav_command_long)
{
...
#if AP_FENCE_ENABLED
MAV_RESULT AP_OpenMV_MAVliteMsgHandler::handle_command_do_fence_enable(const mavlink_command_long_t &mav_command_long)
{
 ...
 * for FrSky SPort Passthrough (OpenTX) protocol (X-receivers)
 */
void AP_OpenMV_MAVliteMsgHandler::handle_param_request_read(const AP_OpenMV_MAVlite_Message &rxmsg)
{
    float param_value;
 ...
 * for FrSky SPort Passthrough (OpenTX) protocol (X-receivers)
 */
void AP_OpenMV_MAVliteMsgHandler::handle_param_set(const AP_OpenMV_MAVlite_Message &rxmsg)
{
    float param_value;
    char param_name[AP_MAX_NAME_SIZE+1];
 ...
 */
MAV_RESULT AP_OpenMV_MAVliteMsgHandler::handle_command_preflight_reboot(const mavlink_command_long_t &mav_command_long)
{
 ...
 * for FrSky SPort Passthrough (OpenTX) protocol (X-receivers)
 */
void AP_OpenMV_MAVliteMsgHandler::process_message(const AP_OpenMV_MAVlite_Message &rxmsg)
{
    switch (rxmsg.msgid) {
 ...
 */
bool AP_OpenMV_MAVliteMsgHandler::send_message(AP_OpenMV_MAVlite_Message &txmsg)
{
