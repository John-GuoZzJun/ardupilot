...
#include "AP_OpenMV_MAVlite.h"
#include "AP_OpenMV.h"
#include "AP_OpenMV_MAVlite_Message.h"

#if HAL_WITH_FRSKY_TELEM_BIDIRECTIONAL

class AP_OpenMV_MAVliteMsgHandler {
public:

    FUNCTOR_TYPEDEF(send_mavlite_fn_t, bool, const AP_OpenMV_MAVlite_Message &);
    AP_OpenMV_MAVliteMsgHandler(send_mavlite_fn_t send_fn) :
        _send_fn(send_fn) {}

    void process_message(const AP_OpenMV_MAVlite_Message &rxmsg);

private:
    // mavlite messages tx/rx methods
    bool send_message(AP_OpenMV_MAVlite_Message &txmsg);

    // gcs mavlite methods
    void handle_param_request_read(const AP_OpenMV_MAVlite_Message &rxmsg);
    void handle_param_set(const AP_OpenMV_MAVlite_Message &rxmsg);

    void handle_command_long(const AP_OpenMV_MAVlite_Message &rxmsg);
